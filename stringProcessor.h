...
const int32_t get_str(const char msg, char* str, int32_t limit)
{
    (void)msg;
    (void)limit;
    if (str != NULL) {
...
int32_t strcmpp(const char* fStr, char* sStr)
{
    while (*fStr && (*fStr == *sStr)) {
        fStr++;
        sStr++;
    }
    return (int32_t)(*(const unsigned char*)fStr - *(const unsigned char*)sStr);
}
